...
def check_nodejs_version(skip_ui=False):
    ...
    # Check npm exists
    npm_result = subprocess.run(["npm.cmd", "--version"], capture_output=True, text=True)
...
def check_sam_cli():
    ...
    sam_result = subprocess.run(["sam.cmd", "--version"], capture_output=True, text=True)
...
def sam_build():
    ...
    run_command(["sam.cmd", "build", "--parallel", "--use-container"])
